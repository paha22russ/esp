...
class TemperaturePanel(ctk.CTkFrame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.home_frame.grid(row=1, column=2, padx=10, pady=10, sticky="nsew")
        self.home_label = ctk.CTkLabel(self.home_frame, text="Дом", font=ctk.CTkFont(size=12))
        self.home_label.pack(pady=(10, 5))
        self.home_value = ctk.CTkLabel(self.home_frame, text="--°C", font=ctk.CTkFont(size=28, weight="bold"))
        self.home_value.pack(pady=5)
        self.home_status = ctk.CTkLabel(self.home_frame, text="", font=ctk.CTkFont(size=10))
        self.home_status.pack(pady=(0, 5))
        
        # Целевая температура дома (для режима Комфорт)
        self.target_home_frame = ctk.CTkFrame(self)
        self.target_home_frame.grid(row=2, column=2, padx=10, pady=10, sticky="nsew")
        self.target_home_label = ctk.CTkLabel(self.target_home_frame, text="Цель дома", font=ctk.CTkFont(size=10))
        self.target_home_label.pack(pady=(5, 2))
        self.target_home_value = ctk.CTkLabel(self.target_home_frame, text="--°C", font=ctk.CTkFont(size=16))
        self.target_home_value.pack(pady=2)
        
        # Уставка
        self.setpoint_frame = ctk.CTkFrame(self)
        self.setpoint_frame.grid(row=1, column=3, padx=10, pady=10, sticky="nsew")
        self.setpoint_label = ctk.CTkLabel(self.setpoint_frame, text="Уставка", font=ctk.CTkFont(size=12))
    ...
    def update_data(self, data: Dict):
        ...
        home_lwt_online = data.get('homeTempSensorLWTOnline', False)
        if home_temp is not None and home_lwt_online:
            self.home_value.configure(text=f"{home_temp:.1f}°C")
            color = processor.get_temperature_color(home_temp, min_temp=15, max_temp=30)
            self.home_value.configure(text_color=color)
            self.home_status.configure(text="● Онлайн", text_color="#50C878")
        else:
            self.home_value.configure(text="offline", text_color="#808080")
            self.home_status.configure(text="● Офлайн", text_color="#808080")
        
        # Уставка (для режима Авто)
        work_mode = data.get('workMode', 0)
        if work_mode == 0:
            setpoint = data.get('setpoint', 0.0)
            self.setpoint_value.configure(text=f"{setpoint:.1f}°C")
        else:
            # В режиме Комфорт показываем целевую температуру дома
            target_home_temp = data.get('targetHomeTemp')
            if target_home_temp is not None:
                self.setpoint_value.configure(text=f"{target_home_temp:.1f}°C")
            else:
                self.setpoint_value.configure(text="--°C")
        
        # Целевая температура дома (для режима Комфорт)
        if work_mode == 1:
            target_home_temp = data.get('targetHomeTemp')
            if target_home_temp is not None:
                self.target_home_value.configure(text=f"{target_home_temp:.1f}°C", text_color="#4A90E2")
            else:
                self.target_home_value.configure(text="--°C", text_color="#808080")
        else:
            self.target_home_value.configure(text="--", text_color="#808080")
