...
class StatsPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        # Статистика вентилятора
        self.fan_stats_frame = ctk.CTkFrame(self)
        self.fan_stats_frame.grid(row=3, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")
        self.fan_stats_label = ctk.CTkLabel(self.fan_stats_frame, text="Статистика вентилятора", font=ctk.CTkFont(size=14, weight="bold"))
        self.fan_stats_label.pack(pady=(10, 5))
        
        fan_stats_inner = ctk.CTkFrame(self.fan_stats_frame)
        fan_stats_inner.pack(pady=5)
        
        self.fan_stats_total = ctk.CTkLabel(fan_stats_inner, text="Всего: --", font=ctk.CTkFont(size=11))
        self.fan_stats_total.pack(side='left', padx=10)
        self.fan_stats_daily = ctk.CTkLabel(fan_stats_inner, text="За сегодня: --", font=ctk.CTkFont(size=11))
        self.fan_stats_daily.pack(side='left', padx=10)
        self.fan_stats_cycles = ctk.CTkLabel(fan_stats_inner, text="Циклов: --", font=ctk.CTkFont(size=11))
        self.fan_stats_cycles.pack(side='left', padx=10)
        
        # Время и дата
        self.time_frame = ctk.CTkFrame(self)
        self.time_frame.grid(row=4, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")
    ...
    def update_data(self, data: Dict):
        ...
        # Статистика вентилятора
        fan_stats_total_time = data.get('fanStatsTotalWorkTime', 0)
        fan_stats_daily_time = data.get('fanStatsDailyWorkTime', 0)
        fan_stats_cycles = data.get('fanStatsCycleCount', 0)
        
        total_hours = fan_stats_total_time // 3600
        total_minutes = (fan_stats_total_time % 3600) // 60
        daily_hours = fan_stats_daily_time // 3600
        daily_minutes = (fan_stats_daily_time % 3600) // 60
        
        self.fan_stats_total.configure(text=f"Всего: {total_hours}ч {total_minutes}м")
        self.fan_stats_daily.configure(text=f"За сегодня: {daily_hours}ч {daily_minutes}м")
        self.fan_stats_cycles.configure(text=f"Циклов: {fan_stats_cycles}")
