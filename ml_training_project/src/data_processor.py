    ...
    def parse_message(data: Dict) -> Optional[Dict]:
        ...
        try:
            return {
                ...
                'homeTemp': DataProcessor._safe_float(data.get('homeTemp'), None) if data.get('homeTemp') is not None and data.get('homeTempSensorLWTOnline') else None,
                ...
                # Режим работы
                'workMode': DataProcessor._safe_int(data.get('workMode'), 0),  # 0 = Авто, 1 = Комфорт
                'workModeName': str(data.get('workModeName', 'Авто')),
                ...
                # Настройки Comfort режима
                'targetHomeTemp': DataProcessor._safe_float(data.get('targetHomeTemp'), None),
                'comfortState': str(data.get('comfortState', '')),
                
                # Статус датчика температуры дома
                'homeTempSensorValid': DataProcessor._safe_bool(data.get('homeTempSensorValid'), False),
                'homeTempSensorLWTOnline': DataProcessor._safe_bool(data.get('homeTempSensorLWTOnline'), False),
                ...
                'boilerExtinguished': DataProcessor._safe_bool(data.get('boilerExtinguished'), False),
                'ignitionInProgress': DataProcessor._safe_bool(data.get('ignitionInProgress'), False),
                'ignitionElapsed': DataProcessor._safe_int(data.get('ignitionElapsed'), 0),
                'ignitionStartTemp': DataProcessor._safe_float(data.get('ignitionStartTemp'), 0.0),
                'ignitionTempIncrease': DataProcessor._safe_float(data.get('ignitionTempIncrease'), 0.0),
                
                # Статистика работы вентилятора
                'fanStats': data.get('fanStats', {}),
                'fanStatsTotalWorkTime': DataProcessor._safe_int(data.get('fanStats', {}).get('totalWorkTime'), 0) if isinstance(data.get('fanStats'), dict) else 0,
                'fanStatsDailyWorkTime': DataProcessor._safe_int(data.get('fanStats', {}).get('dailyWorkTime'), 0) if isinstance(data.get('fanStats'), dict) else 0,
                'fanStatsCycleCount': DataProcessor._safe_int(data.get('fanStats', {}).get('cycleCount'), 0) if isinstance(data.get('fanStats'), dict) else 0,
                'fanStatsDailyCycleCount': DataProcessor._safe_int(data.get('fanStats', {}).get('dailyCycleCount'), 0) if isinstance(data.get('fanStats'), dict) else 0,
                'fanStatsCurrentWorkTime': DataProcessor._safe_int(data.get('fanStats', {}).get('currentWorkTime'), 0) if isinstance(data.get('fanStats'), dict) else 0,
